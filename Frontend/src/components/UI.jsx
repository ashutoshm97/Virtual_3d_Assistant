    ...
    if (!loading && !message) {
      try {
        if (text.includes("reminder")) {
          ...
          const response = await fetch(`${config.paths.backendUrl}/createEvent`, {
            method: "POST",
        ...
        if (text.includes("send an email")) {
          ...
          const response = await fetch(`${config.paths.backendUrl}/sendEmail`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (containsKBKeyword) {
          const response = await fetch(`${config.paths.backendUrl}/rag`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
const downloadTextFile = (content) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.paths.backendUrl}/transcript`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    ...
    <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
      <div className="self-start backdrop-blur-md bg-white bg-opacity-50 p-4 rounded-lg">
        <h1 className="font-white text-xl">Donna</h1>
        <p><b>Personalized Virtual Assistant</b></p>
        <ul className="mt-2 text-sm">
          <li>• Using Chatgpt API to provide answers </li>
          <li>• Using knowledge base to provide answers specific to university using prompt "knowldge base"</li>
          <li>• Note Taking through audio input using prompt "note down" at start</li>
          <li>• Transcritpion capability for uploaded audio</li>
          <li>• Sending email to a id with subject with prompt "send an email"</li>
        </ul>
      </div>
